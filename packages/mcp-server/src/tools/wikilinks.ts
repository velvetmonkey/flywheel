...
export function registerWikilinkTools(
  ...
  const SuggestionSchema = z.object({
    ...
    start: z.number().describe('Start position in text (0-indexed)'),
    end: z.number().describe('End position in text (0-indexed)'),
    target: z.string().describe('Path to the target note'),
  });

  const SuggestWikilinksOutputSchema = {
    input_length: z.number().describe('Length of the input text'),
    suggestion_count: z.number().describe('Total number of suggestions found'),
    returned_count: z.number().describe('Number of suggestions returned (may be limited)'),
  ...
  server.registerTool(
    'suggest_wikilinks',
    {
      ...
      inputSchema: {
        ...
        limit: z.number().default(50).describe('Maximum number of suggestions to return'),
        offset: z.number().default(0).describe('Number of suggestions to skip (for pagination)'),
      },
  ...
  const ValidateLinksOutputSchema = {
    ...
    total_links: z.number().describe('Total number of links checked'),
    valid_links: z.number().describe('Number of valid links'),
    broken_links: z.number().describe('Total number of broken links'),
  ...
  server.registerTool(
    'validate_links',
    {
      ...
      inputSchema: {
        ...
        limit: z.number().default(50).describe('Maximum number of broken links to return'),
        offset: z.number().default(0).describe('Number of broken links to skip (for pagination)'),
      },
